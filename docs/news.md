<h3 style="margin-bottom: 1.5rem; font-size: 1.7rem !important;">News</h3>

<div style="display: flex; flex-direction: column; gap: 0;">

  <a href="#" data-md="docs/changelog/realtime-cursors.md" style="display: flex; gap: 24px; padding: 24px 0; border-bottom: 1px solid var(--border); text-decoration: none; color: inherit; cursor: pointer; transition: opacity 0.2s ease;" onmouseover="this.style.opacity='0.75'" onmouseout="this.style.opacity='1'">
    <div style="display: flex; flex-direction: column; align-items: flex-start; gap: 8px; width: 110px; flex-shrink: 0;">
      <span style="font-size: 13px; opacity: 0.5; font-weight: 500;">Sep 23, 2026</span>
    </div>
    <div style="display: flex; flex-direction: column; gap: 6px; max-width: 680px;">
      <strong style="font-size: 16px; font-weight: 450; line-height: 1.3;">Introducing Armadillo Learn</strong>
      <p style="margin: 0; font-size: 14px; opacity: 0.6; line-height: 1.6;">Armadillo Learn is an educational platform by Armadillo Software, previously known as Newton's Lost Apple. We build open resources for teachers, students, and anyone eager to learn something new. Take a look at our materials and start exploring.</p>
    </div>
  </a>
</div>